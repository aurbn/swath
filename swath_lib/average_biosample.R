#
average.ms <- function(data= NULL, data.file= NULL, output.file= NULL, return.table= TRUE, write.file= !is.null(output.file)){
  ...
  data <- average.tech(data)
  
  data[, c('biosample_mean','biosample_se','biosample_cv'):= list(mean(intensity), se_corr(intensity), se_corr(intensity)/mean(intensity)), by=c('fragment_id', 'biosample')]
  data <- unique(data[, c('intensity', 'tech_id', 'tech', 'sample_code'):= NULL])
  setnames(data, 'tech_mean', 'intensity')

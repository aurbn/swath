#
fragment.score <- function(data= NULL, data.file= NULL, measure_id= 'bio_sample', output.file= NULL, return.table= TRUE, write.file= !is.null(output.file)){
  ...
  keys <- c('fragment_id', measure_id)
  setkeyv(data, keys)
  
  data[, precursor_score:= sum(intensity), by= keys]
  data[, frg_num:= ulength(fragment_id), by= fragment_id]
  data <- unique(data[, c('protein_id', 'peptide_id', 'precursor_id', 'fragment_id', measure_id, 'fragment_score'), with= FALSE])

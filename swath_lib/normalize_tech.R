#
normalize.tech <- function(data= NULL, measure.id= 'fragment_id', value.var= 'intensity', coef= NULL, by_group= TRUE, data.file= NULL, coef.file= NULL, output.file= NULL, return.table= TRUE, return.coef= FALSE, write.file= !is.null(output.file), output.coef= NULL, write.coef= !is.null(output.coef)){
  ...
  if(is.null(coef)&is.null(coef.file)){
    if(by_group){
      coef <- scaleNorm(dt= average.ms(data), measure.id= measure.id, value= value.var, rep.id= 'tech_id', group.id= 'bio_sample')
    } else {
      coef <- scaleNorm(dt= average.ms(data), measure.id= measure.id, value= value.var, rep.id= 'tech_id', group.id= NULL)
    }
...
normalize.tech.coef <- function(data= NULL, data.file= NULL, by_group= TRUE, output.coef= NULL, write.coef= !is.null(output.coef)){
  # Alias for special default filename arguments
  normalize.tech(data= data, coef= NULL, data.file= data.file, by_group= by_group, coef.file= NULL, output.file= NULL, return.table= FALSE, return.coef= TRUE, write.file= FALSE, write.coef= write.coef, output.coef= output.coef)  
}
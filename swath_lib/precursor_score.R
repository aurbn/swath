#
precursor.score <- function(data= NULL, data.file= NULL, measure_id= 'bio_sample', output.file= NULL, return.table= TRUE, write.file= !is.null(output.file)){
  ...
  keys <- c('precursor_id', measure_id)
  setkeyv(data, keys)
  
  data[, precursor_score:= sum(intensity), by= keys]
  data[, frg_num:= ulength(fragment_id), by= precursor_id]
  data <- unique(data[, c('protein_id', 'peptide_id', 'precursor_id', 'frg_num', measure_id, 'precursor_score'), with= FALSE])
